...
if [[ "$1" == "--setPhysicsModelParameters" ]]; then 
    BSMOPT="$1"; shift; BSMOPT="$BSMOPT $1"; shift; 
    if [[ "$1" == "--freezeNuisances" ]]; then
	BSMOPT="$BSMOPT $1"; shift; BSMOPT="$BSMOPT $1"; shift; 
    fi
fi
...
function run {
    ...
    BSP=${1/.root};
    OPTS="$OPTIONS --X-rtd TMCSO_AdaptivePseudoAsimov $BSMOPT"
    ...
    if test -f $1; then
         echo "Running $TASK asymptotic frequentist limits for $NAM at $MASS. ";
         [[ "$COMBINE_NO_LOGFILES" != "1" ]] && DO_LOG="tee ${1/.root/.log}.$MYWHAT" || DO_LOG="dd of=/dev/null" 
	 echo "Command line is: combine -M Asymptotic $* -n ${NAM}_${MYWHAT} -m ${MASS} ${OPTS} 2>&1 | ${DO_LOG}"
