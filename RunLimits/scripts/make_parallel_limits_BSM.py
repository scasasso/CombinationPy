...
def parseOptions():
    ...
    parser.add_option('-n', '--queue', dest='queue', type='string', default="1nd",    help='LSF queue: 1nd, 2nd, 1nw ...')
...
def submit():
    ...
    for line in open(opt.massfile,'r'):
        ...
        if opt.scheduler == "lsf" :
            cmd = 'bsub -R "select[type==SLC6_64]" -q '+str(opt.queue)+' -o '+str(f[0])+'/lsflog_stdout_'+opt.method+'.txt -e '+str(f[0])+'/lsflog_stderr_'+opt.method+'.txt makeLimits.BSM.lsf.sh '+opt.method+' '+str(f[0])+' '+opt.tool+' '+opt.options
